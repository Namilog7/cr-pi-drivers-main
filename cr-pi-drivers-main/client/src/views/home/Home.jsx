    ...
    return(
        ...
        <div className={style.contSearch}>
            <div className={style.filters}>
                <select name="filter" id="filter" onChange={()=>handleChange(event,filterTeam)}>
                    <option value="" disabled selected >Selected Team</option>
                    {teams && teams.map(team=>{
                        return <option key={team} value={team} id={team}>{team}</option>
                    })}
                </select>
                <select name="filter2" id="filter2" onChange={()=>handleChange(event,filByOr)}>
                    <option value= "" disabled selected>Selected option</option>
                    <option value="api" >Api</option>
                    <option value="db">DataBase</option>
                </select>
                <select name="filter3" id="filter3" onChange={()=>handleChange(event,orderByAlpha)}>
                    <option value="" disabled selected>Selected option</option>
